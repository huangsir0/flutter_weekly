...
class _GestureWidgetState extends State<GestureWidget> {
  ...
  _GestureWidgetState() {
    ...
    if (_ctrlOffset1 == null) {
      ...
      _ctrlOffset1 =
          Offset(Screen.screenWidthDp - 90, Screen.screenHeightDp / 3 - 60);
    }
  } //控制点2
        ...
        GestureDetector(
          onPanUpdate: (DragUpdateDetails details) {
            setState(() {
              ...
              double ctrl0Length = math.sqrt(
                  (math.pow((moveOffset.dx - _ctrlOffset0.dx).abs(), 2) +
                      (math.pow(
                          (moveOffset.dy - _ctrlOffset0.dy).abs(), 2)))); //勾股定理
              double ctrl1Length = math.sqrt(
                  (math.pow((moveOffset.dx - _ctrlOffset1.dx).abs(), 2) +
                      (math.pow(
                          (moveOffset.dy - _ctrlOffset1.dy).abs(), 2)))); //勾股定理
...
class BezierWidget extends StatefulWidget {
  ...
  BezierWidget(
      this._type, this._moveOffset, this._ctrlOffset0, this._ctrlOffset1);
...
class _BezierWidgetState extends State<BezierWidget> {
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      painter: BezierExamplePainter(this.widget._type, this.widget._moveOffset,
          this.widget._ctrlOffset0, this.widget._ctrlOffset1),
    );
  }
}
  ...
  void paint(Canvas canvas, Size size) {
    ...
    switch (_type) {
      ...
      case 2:
        {
          ...
          _linePath.quadraticBezierTo(this._moveOffset.dx, this._moveOffset.dy,
              _endOffset.dx, _endOffset.dy);
      ...
      case 3:
        {
          _linePath.moveTo(_startOffset.dx, _startOffset.dy);
          _linePath.cubicTo(_ctrlOffset0.dx, _ctrlOffset0.dy, _ctrlOffset1.dx,
              _ctrlOffset1.dy, _endOffset.dx, _endOffset.dy);
          canvas.drawCircle(_endOffset, _r, _pointPaint);
